...
const generateSections = async (chapters, count) => {
    ...
    const chaptersString = JSON.stringify(chapters);
    const promptText = [
    ...
    const sections = result.sections.map((chapterSections) => chapterSections.map((s, index) => ({
...
//# sourceMappingURL=generateSections.js.map