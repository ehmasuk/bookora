  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-4">Review all content and provide a title for your new book.</p>
      </div>

      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleCreateBook)} className="space-y-6">
          <FormField
          ...
          <div className="space-y-6">
            {chapters.map((chapter: { title: string; summary: string; position: number }, chapterIndex: number) => (
              <div key={chapterIndex} className="border rounded-lg p-4 space-y-4 bg-muted/20">
                ...
                {sections[chapterIndex] && sections[chapterIndex].length > 0 && (
                  <div className="pl-4 border-l-2 border-muted space-y-1">
                    {sections[chapterIndex].map((section: { title: string }, sectionIndex: number) => (
